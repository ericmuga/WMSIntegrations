  ...
  return [
    {
      ...
      production_order_no: getProcessDetails(items[0].process_code).production_order_series+'_'+responseData.id,

      ItemNo: responseData.item_code,
      Quantity: parseFloat(responseData.total_weight), // Convert string to number for Quantity
      uom: 'KG', // Default to "KG" if not provided
      LocationCode: getProcessDetails(responseData.process_code).output_location,
      ...
      ProductionJournalLines: items.flatMap((item, index) => {
        const processDetails = getProcessDetails(item.process_code);

        if (!processDetails) {
          throw new Error(`Process name ${item.process_code} not found in lookup`);
        }

        return [
          {
            ItemNo: item.item_code,
            Quantity: parseFloat(item.total_weight), // Convert string to number for Quantity
          ...
          },
          {
            ...
            Quantity:Math.round((parseFloat(item.total_weight) / (1 - parseFloat(processDetails.process_loss))) * 100) / 100, // Convert string to number for Quantity
            uom: item.uom || "KG", // Assume default "KG" if not provided
